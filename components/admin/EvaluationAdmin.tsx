      ...
      <header className="sticky top-0 z-30 glass border-b border-border">
        <div className="mx-auto max-w-7xl px-6 h-16 flex items-center gap-4">
          ...
          <div className="hidden sm:block">
            <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground font-mono"><span className="text-destructive">o</span> Dossiê técnico</div>
          ...
          <>
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              <p className="mt-2 text-muted-foreground">
                {projeto.proponente || "Proponente não informado"} - Auditor: {projeto.auditor || "não definido"} - Criado em {formatDate(projeto.criadoEm?.slice(0, 10))}
              </p>
            </motion.div>
            ...
            <div className="mt-8 grid lg:grid-cols-3 gap-6">
              <section className="lg:col-span-2 bg-card border border-border rounded-3xl overflow-hidden">
                <div className="px-6 py-5 border-b border-border">
                  <div className="text-[10px] uppercase tracking-[0.2em] font-mono text-muted-foreground">Plano de execução</div>
                    ...
                    <article key={atividade.id} className="rounded-2xl border border-border bg-secondary/30 p-4">
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-1 text-sm text-muted-foreground">{atividade.descricao || "Sem descrição"}</div>
                        </div>
                      ...
                      <div className="mt-4 grid sm:grid-cols-3 gap-3 text-xs text-muted-foreground">
                        ...
                        <Info label="Início" value={formatDate(atividade.inicio)} />
              ...
              <aside className="space-y-4">
                <div className="bg-card border border-border rounded-3xl p-6">
                  ...
                  <div className="mt-4 text-sm space-y-2">
                    ...
                    <div className="flex justify-between gap-4"><span className="text-muted-foreground">Evidências</span><span>{projeto.evidencias.length}</span></div>
                    <div className="flex justify-between gap-4"><span className="text-muted-foreground">Auditor</span><span className="truncate">{projeto.auditor || "Não definido"}</span></div>
                  </div>
                </div>

                <div className="bg-card border border-border rounded-3xl p-6">
                  <div className="text-[10px] uppercase tracking-[0.2em] font-mono text-muted-foreground mb-3">Evidências</div>
                  <div className="space-y-2">
                    {projeto.evidencias.length === 0 && <div className="text-sm text-muted-foreground">Nenhuma evidência enviada.</div>}
                    {projeto.evidencias.map((evidencia) => (
                      <div key={evidencia.id} className="rounded-xl border border-border bg-secondary/40 p-3">
                        <div className="text-sm font-medium">{evidencia.atividade || `Evidência ${evidencia.id}`}</div>
                        <div className="text-xs text-muted-foreground mt-1">{evidencia.descricao || "Sem descrição"}</div>
